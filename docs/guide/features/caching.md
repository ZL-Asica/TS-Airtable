...
Caching is completely optional. Whether you _should_ turn it on depends on your access patterns and consistency requirements.
...
In short: turn caching on where it clearly reduces repeated reads and latency, keep TTLs modest, and lean on `deleteByPrefix` invalidation when you can. For everything else, it’s perfectly fine to leave caching disabled.
